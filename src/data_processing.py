...
class DataProcessor:
    ...
    def _get_files_dict(self):
        ...
        for filename in sorted(
            os.listdir(self.pings_folder), key=lambda x: int(x.split("-")[1])
        ):
            ...
            files_dict[idx] = {
                ...
                "motion": os.path.join(
                    self.motion_folder, f'{filename.split(".")[0]}.csv'
                ),
                "vehicle_pos": os.path.join(
                    self.vehicle_pos_folder, f'{filename.split(".")[0]}.nav'
                ),
    ...
    def _parse_motion(self, idx):
        ...
        motion_df["datetime"] = pd.to_datetime(
            motion_df["Date"] + motion_df["Time"], format="%Y-%m-%d%H:%M:%S.%f"
        )
    ...
    def _parse_vehicle_pos(self, idx):
        ...
        vehicle_pos_df = pd.read_csv(
            filename, sep=r",\s*", engine="python", dtype={"DATE": str, "TIME": str}
        )
        vehicle_pos_df["datetime"] = pd.to_datetime(
            vehicle_pos_df["DATE"] + vehicle_pos_df["TIME"], format="%Y%m%d%H:%M:%S.%f"
        )
        vehicle_pos_df.rename(
            columns={"EASTING": "easting", "NORTHING": "northing"}, inplace=True
        )
    ...
    def _parse_raw_pings(self, idx):
        ...
        df = pd.read_csv(
            filename, sep="\t", dtype={"yyyy": str, "mmddhhmm": str, "ss.ss": str}
        )
    ...
    def get_processed_data_df(self, idx):
        ...
        vehicle_pos_df = self._parse_vehicle_pos(idx)[
            ["datetime", "easting", "northing"]
        ]
        motion_df = pd.merge_asof(
        ...
        pings_df = pd.merge_asof(
            pings_df.sort_values("datetime"),
            motion_df.sort_values("datetime"),
            on="datetime",
            direction="nearest",
        )
        ...
        processed_df = pd.merge(
            pings_df,
            angle_quality_df,
            on=["scan_no", "beam_id", "X", "Y", "Z"],
            how="outer",
        )
        processed_df = pd.merge(
            processed_df, cleaned_df, on=["X", "Y", "Z"], how="outer"
        )

        # Drop any duplicated rows
        processed_df.drop_duplicates(
            subset=["Ping No", "Beam No", "X", "Y", "Z"], inplace=True
        )
    ...
    def _processed_data_df_to_numpy_dict(self, processed_df):
        ...
        data = {
            ...
            "intensity": pivot["intensity"].to_numpy().astype(np.int32),
            "angle": pivot["angle"].to_numpy().astype(np.float32),
            "quality": pivot["quality"].to_numpy().astype(np.int32),
            "rejected": pivot["rejected"].to_numpy().astype(bool),
    ...
    def plot_numpy_data(
        ...
        fig, axes = plt.subplots(
            num_rows, num_columns, figsize=(num_columns * 3, num_columns * 3)
        )
        ...
        if savefig:
            ...
            figname = (
                f"{filename}{suffix}-filtered.png"
                if filter_rejected
                else f"{filename}{suffix}.png"
            )
    ...
    def merge_all_npz_data(self):
        ...
        for k, v in self.files_dict.items():
            if not os.path.exists(v["out"]):
                self.logger.error(
                    f"Processed data not found for file idx {k}. Run process_folder() first."
                )
    ...
    def create_patches(self, data_path, pings_per_patch, beams_per_patch):
        ...
        if not os.path.exists(patches_folder):
            ...
            self.logger.warning(
                f"Patches folder {patches_folder} already exists. Overwriting..."
            )

        self.logger.info(
            f"Creating patches with size ({pings_per_patch}, {beams_per_patch})..."
        )
        ...
        for i in tqdm(range(0, num_pings, pings_per_patch)):
            for j in tqdm(range(0, num_beams, beams_per_patch)):
                patch = {
                    k: v[i : i + pings_per_patch, j : j + beams_per_patch]
                    for k, v in all_data.items()
                }
                ...
                np.savez(
                    os.path.join(patches_folder, f"patch_{num_patches}.npz"), **patch
                )
                num_patches += 1
                del patch
        self.logger.info(
            f"Created {num_patches} patches\n" f"saved to {patches_folder}"
        )
